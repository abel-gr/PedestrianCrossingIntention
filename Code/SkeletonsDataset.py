...
class SkeletonsDataset(Dataset):
    ...
    """
    def __init__(self, csv_path, numberOfJoints=25, normalization='minmax', norm_precomputed_values=None, target='cross',
                 info='spatial', remove_undetected=True, use_confidence=True, target_unique_frame=True, transform=None):
        ...
        self.target_unique_frame = target_unique_frame
        
        
        if numberOfJoints == 25: # OpenPose BODY_25
        ...
        else: # AlphaPose COCO 18 keypoints
    ...
    def processData(self):
        ...
        if self.remove_undetected:
            ...
            # self.loadedData = self.loadedData[self.loadedData['occlusion'] != 'full']
            # self.loadedData.loc[self.loadedData['skeleton_detected']==False, ['skeleton']] = '[' + ('[0.0, 0.0, 0.0], ' * self.numberOfJoints)[0:-2] + ']'
            # self.loadedData.loc[self.loadedData['occlusion'] == 'full', ['skeleton']] = '[' + ('[0.0, 0.0, 0.0], ' * self.numberOfJoints)[0:-2] + ']'
        else:
            self.loadedData['skeleton'] = self.loadedData['skeleton'].replace(np.nan, '[' + ('[0.0, 0.0, 0.0], ' * self.numberOfJoints)[0:-2] + ']')
        ...
        pedestriansIDs = self.loadedData['ped_id'].tolist()
        frameIDs = self.loadedData['frame'].tolist()
        ...
        for i, skeleton in enumerate(skeletonData):
            ...
            videoID = videoIDs[i]
            pedestrianID = pedestriansIDs[i]
            frameID = frameIDs[i]
            if videoID in self.original_skeletons:
                if frameID in self.original_skeletons[videoID]:
                    self.original_skeletons[videoID][frameID][pedestrianID] = np.copy(jointCoords)
                else:
                    self.original_skeletons[videoID][frameID] = {pedestrianID: (np.copy(jointCoords))}
            else:
                self.original_skeletons[videoID] = {frameID: {pedestrianID: (np.copy(jointCoords))}}
        ...
        if self.graphInfo == 'spatial':
            ...
            unique_pedestrianIDs = pd.unique(self.loadedData['ped_id']).tolist()
            
            for pedID in unique_pedestrianIDs:
                ...
                # For each skeleton of the dataset
                for j, _ in enumerate(skeletonData):
                    
                    videoID_j = videoIDs[j]
                    
                    pedestrianID_j = pedestriansIDs[j]
                                            
                    if pedID != pedestrianID_j:
                        continue
                        
                    y_values_ped.append(y_values[j].numpy())
                    x_values_ped.append(x_values[j])
                    
                    id_video_i = videoID_j
                        
                
                y_values_ped = np.array(y_values_ped)
                #x_values_ped = torch.stack(x_values_ped)
                                
                for i in range(numFrames - 1, len(x_values_ped)):

                    targets_i = y_values_ped[i:i + 1] if self.target_unique_frame else y_values_ped[i - numFrames + 1:i + 1]
                    ...
                    x_temp = x_values_ped[i - numFrames + 1:i + 1]
                    ...
                    if len(x_temp) == numFrames:
                        ...
                        data_element = Data(x_temporal=x_temp, y=y_value_i, label=label_i, edge_index=edge_index,
                                            num_nodes=x_temp[0].shape[0], edge_weight=self.edge_weights, videoID=id_video_i)
    ...
    def showSkeleton(self, videoNum=0, frameNum=0, textSize=14, showLegend=True, frameImage=None, normalizedSkeletons=True, 
                     title='Skeleton preview in 2D', show=True, predictions=None, groundtruths=None):
        
        parts = list(self.body_parts.keys())

        node_coords = {}

        fig = plt.figure(figsize=(10,7), dpi=150) if show else Figure(figsize=(10,7), dpi=150)
        ...
        if normalizedSkeletons:
            ped_skeletons = self.data[videoNum].x_temporal[frameNum][:, 0:2].tolist()
        else:
            video_skeletons = self.original_skeletons[videoNum]
            
            if frameNum in video_skeletons:
                ped_skeletons = video_skeletons[frameNum]
                
                
        for i, skeleton in enumerate(ped_skeletons.values()):
            
            skeleton = skeleton[:, 0:2].tolist()
            ...
            if prediction is not None:
                ...
                if groundtruth is not None:
                    ...
                    prediction = prediction + " \u2714" if prediction == groundtruth else prediction + " \u2716"
